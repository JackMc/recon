class ScreenshotJob < ApplicationJob
  ...
  def perform(http_probe_id:)
    ...
    browser.goto(url)
    filesafe_website_name_with_extension = "#{url.gsub(/[:\.\/]+/, '_')}.png"
    filename = "tmp/screenshots/#{filesafe_website_name_with_extension}"
    browser.screenshot(path: filename)
    browser.quit

    http_probe.screenshot.attach(io: File.open(filename), filename: filesafe_website_name_with_extension, content_type: 'image/png')
  end
end
